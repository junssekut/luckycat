    ...
    <template x-for="toast in toasts" :key="toast.id">
        ...
        @else
            ...
            <i x-text="toast.message"
                class="inline-block select-none not-italic px-6 py-2 pt-4 rounded-none shadow-lg text-sm w-full {{ $alignment->is('bottom') ? 'mt-3' : 'mb-3' }} border border-black font-Geomisans uppercase"
                :class="toast.select({
                    error: 'bg-red-500',
                    info: 'bg-gray-200',
                    success: 'bg-luckycat-300',
